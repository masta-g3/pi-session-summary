...
`stage` is model-inferred semantic activity, attention is an explicit human-action claim, and Hub process liveness is authoritative separately. Ordinary waiting does not imply attention. Hub renders the attention reason only in selected workflow cards and stage-grouping lanes for waiting/idle Active rows; canonical workflow metadata is not required because non-workflow Active sessions render in `OTHER ACTIVE`. Compact project rows do not show the marker. Raw prompts, tool arguments, command output, the full checklist, completion evidence/usage telemetry, and conversation snippets stay out of the metadata file.
